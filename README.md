...
## 8일차
- WPF 개인 프로젝트 포트폴리오 작업 [OpenAPI를 활용한 김해시 식당 정보 제공 앱]
    - 데이터포털 API 사용
        - [x] 음식점 분류값을 입력하면 해당 입력값에 맞는 결과 보여주기
			- (ex) 한식 입력 시, 카테고리가 한식인 식당들의 정보 요청 > API로 받아온 데이터 보여주기
		- [x] 즐겨찾기 추가 버튼으로 원하는 식당만 Restaurant DB에 저장할 수 있음
		- [x] 즐겨찾기 보기 버튼 > 즐겨찾기 한 식당 카테고리별로 볼 수 있음
		- [x] 상세정보 글자 다 보여줄 수 있는 속성 찾기
    
    - 기능
ㄴ        - 한식, 중식, 일식 등 사용자가 먹고싶은 음식의 종류를 입력하면, 해당 입력값에 맞는 식당 정보 제공
            - 식당 분류, 업소명, 지역, 주소, 상세 정보, 휴무일, 대표전화 정보 제공
        - 가고 싶은 식당 즐겨찾기 추가 기능 제공
        - 즐겨찾은 식당을 음식 카테고리 별로 열람 가능
        - 해당 식당 클릭 시 지도로 위치 확인 가능

...
- WPF 개인 프로젝트 포트폴리오 작업
    ...
    - 학교와서 수정하기
        - 음식점 분류값을 입력하면 해당 입력값에 맞는 결과 보여주기(원하는 결과들만 DB 저장 => 조회 데이터 DB에 저장하고 즐겨찾기 식당만 따로 보여주기..)
		- 가고 싶은 식당만 저장할 수 있도록 즐겨찾기 버튼 추가
			- 저장 중복 안되게 해줘야함,, 흐음
		- 상세정보 글자 다 보여줄 수 있는 속성 찾기
		- 식당 검색 버튼 다른 형식으로 만들기
		- Row 2 에 기능 추가하기 (조회한 데이터 수 보여주기, 즐겨찾기한 식당 보기 버튼 추가)